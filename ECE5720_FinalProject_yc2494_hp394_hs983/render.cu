...
{
  int *image = (int*) malloc( sizeof(int) * WIDTH * HEIGHT * 3);
  int i;
...
*/
    FILE *fp;
    char *file_path = strcat(itoa(step), ".csv");
    fp = fopen(file_path, "w+"); 
    int i = 0;
    for( i = 0; i < num_of_bodies; i++) {
	
      fprintf(fp, "%f", position[i * 3]);
      fprintf(fp, ",");
      fprintf(fp, "%f", position[i * 3 + 1]);
      fprintf(fp, ",");
      fprintf(fp, "%f", position[i * 3 + 2]);
      fprintf(fp, "\n");
    }
   fclose(fp);
}
